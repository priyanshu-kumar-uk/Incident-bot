...
const Sidebar: React.FC<SidebarProps> = ({ isMobileOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex h-[100dvh] flex-col border-r border-white/10 bg-[#0a0f1a]/95 backdrop-blur-xl transition-all duration-300 ease-in-out select-none ${
          collapsed ? 'w-20' : 'w-[82vw] sm:w-72'
        } ${isMobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} md:static md:h-screen md:w-64`}
      >
        {/* Brand Logo Header */}
        <div
          className={`flex shrink-0 items-center gap-3 border-b border-white/10 px-3 py-4 transition-all duration-300 ease-in-out ${
        ...
        {/* Scrollable Center Area (Navigation Links + Telegram CTA) */}
        <div className="flex flex-1 flex-col justify-between overflow-y-auto min-h-0">
        ...
        {/* User Profile & Logout — Pinned to Bottom with Safe Area Padding */}
        <div className="shrink-0 border-t border-white/10 p-2 pb-6 sm:p-3 sm:pb-3 bg-[#0a0f1a]">
          {!collapsed && user && (
